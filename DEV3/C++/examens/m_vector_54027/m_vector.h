...
void m_vector::validate_index(std::size_t index) const {
    if (index >= size_) {
        throw std::invalid_argument("Invalide size : index : " +
                                    std::to_string(index)
                                    + " and size_ : " + std::to_string(size_));
    }
}

m_vector::m_vector(std::size_t size) :
    size_ { size },
    data_ { std::vector<double> {0}} {
    if (size <= 0) {
        throw std::invalid_argument("Given size is null");
    }
}

m_vector::m_vector(std::size_t size, double value) :
    size_ { size },
    data_ { std::vector<double> {value} } {
    if (size <= 0) {
        throw std::invalid_argument("Given size is null");
    }
}
...
m_vector::m_vector(std::initializer_list<double> data) :
    ...
    data_ { data } {
    if (data.size() <= 0) {
        throw std::invalid_argument("Given size of data is null");
    }
}
...
std::string m_vector::to_string() const {
    std::string formated { "Nombre de composantes : " + std::to_string(
                               size()) + " => (" };
    for (auto const & values : data_) {
        formated += std::to_string(values) + ", ";
    }
    formated.resize(formated.size () - 2);
    formated += ") = " + std::to_string(double(m_vector {data_}));
    return formated;
}

m_vector::operator double() const {
    return norm(m_vector {data()});
}
...
inline void validate_size(const m_vector & lhs,
                          const m_vector & rhs) {
    if (!equal_size(lhs, rhs)) {
        throw std::invalid_argument("Number of composant of lhs is not same than rhs.");
    }
}
...
m_vector operator*(double d, const m_vector & v) {
    ...
    for (const auto & value : v.data()) {
        newData.push_back(value * d);
    }
    return m_vector { newData };
}
...
double operator*(const m_vector & lhs, const m_vector & rhs) {
    ...
    for (std::size_t i = 0; i < lhs.size(); i++) {
        newData.push_back(lhs[i] * rhs[i]);
    }
    return double(m_vector {newData} );
}

double norm(const m_vector & v) {
    double calcul {};
    for (auto const & value : v.data()) {
        calcul += std::pow(value, 2);
    }
    return std::sqrt(calcul);
}
